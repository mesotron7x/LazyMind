...
| `word_groups` | 词汇组 | 后续 |
| `memories` | 记忆 | 后续 |
| `evolution_*` | 进化相关 | 不需要（Desktop 主链路裁剪 Evo） |
...
#### 4.2.3 Phase 1 必需表（auth-service）
...
- SQLite 文件默认不加密；密钥和诊断包安全按 `16-credential-security.md` 与本文件脱敏规则处理，SQLCipher 可作为独立增强项。

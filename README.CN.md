...
LazyRAG 是一个**开箱即用的企业知识库 + RAG 对话平台**，同时内置了一套**自动化 RAG 质量优化闭环（evo）**。
...
这是 LazyRAG 最独特的能力。传统 RAG 系统上线后质量好不好全靠人工排查，evo 模块让系统**自己跑通优化流程**：
...
| 启用 MinerU OCR | `make up LAZYRAG_OCR_SERVER_TYPE=mineru` |
| 启用 PaddleOCR（GPU） | `make up LAZYRAG_OCR_SERVER_TYPE=paddleocr` |
| 外接 Milvus/OpenSearch | `make up LAZYRAG_MILVUS_URI=http://your-milvus:19530 LAZYRAG_OPENSEARCH_URI=https://your-opensearch:9200` |
| 开启存储 Dashboard | `make up LAZYRAG_ENABLE_STORE_DASHBOARDS=1` |
...
- Go 模块：`backend/core` 使用 `module lazyrag/core`

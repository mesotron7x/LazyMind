...
// customizeDatalakeWriteErrorResponseJSON - writes error response in JSON format;
// 自定义 traceID 并返回
func customizeDatalakeWriteErrorResponseJSON(ctx context.Context, w http.ResponseWriter, r *http.Request, err APIError, reqURL *url.URL) {
	// Generate error response.
	var errorResponse APIErrorResponse
	traceID := r.Header.Get(xhttp.SensetimeSaaSTraceID)
	if len(traceID) > 0 {
		errorResponse = getAPIErrorResponse(ctx, err, reqURL.Path, traceID, globalDeploymentID)
	} else {
		errorResponse = getAPIErrorResponse(ctx, err, reqURL.Path, w.Header().Get(xhttp.AmzRequestID), globalDeploymentID)
	}
	// 返回response code更改成httpstatuscode
	type CustomizeErrRes struct {
		Code       int
		Message    string
		BucketName string
		Key        string
	...
	customErrRes := CustomizeErrRes{
		Code:       err.HTTPStatusCode,
		Message:    fmt.Sprintf("%s, %s", errorResponse.Code, errorResponse.Message),
		BucketName: errorResponse.BucketName,
		Key:        errorResponse.Key,
		Resource:   errorResponse.Resource,
		Region:     errorResponse.Region,
		RequestID:  errorResponse.RequestID,
		HostID:     errorResponse.HostID,
	}
	encodedErrorResponse := encodeResponseJSON(customErrRes)
	writeResponse(w, err.HTTPStatusCode, encodedErrorResponse, mimeJSON)
}

...
func serverHandleCmdArgs(ctx *cli.Context) {
	...
	if globalCLIContext.GBucket != globalCustomizedDatalakeBucket {
		globalCustomizedDatalakeBucket = globalCLIContext.GBucket
	}

	globalMountDir := "/tmp/fakeEndpoint"
	if len(globalCLIContext.MountDir) > 0 {
		globalMountDir = globalCLIContext.MountDir
	}
	globalEndpoints, setupType, err = createServerEndpoints(globalCLIContext.Addr, globalMountDir)

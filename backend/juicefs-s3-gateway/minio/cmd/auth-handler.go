...
func isRequestCustomizedDatalake(r *http.Request) bool {
	return ((len(r.Header.Get(xhttp.SensetimeSaaSUserID)) > 0 && len(r.Header.Get(xhttp.SensetimeSaaSTenantID)) > 0) ||
		(len(r.Header.Get(xhttp.SensetimeOneAPIUserID)) > 0 && len(r.Header.Get(xhttp.SensetimeOneAPIUserName)) > 0))
}
...
func getRequestAuthType(r *http.Request) authType {
	if isRequestCustomizedDatalake(r) {
		return authTypeDatalake
...
func setAuthHandler(h http.Handler) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if isSupportedS3AuthType(aType) {
			...
			h.ServeHTTP(w, r)
			return
		} else if aType == authTypeDatalake {
			h.ServeHTTP(w, r)
			return
		}
...
// isDatalakeActionAllowed 为datalake storage专用鉴权逻辑
// 需要校验的内容包括，远端bucketname和ak sk的合法性
// 用户侧token、ak sk的合法性
// 用户侧objectname操作权限合法性
func isDatalakeActionAllowed(ctx context.Context, bucketName, objectName string, r *http.Request, action iampolicy.Action) (s3Err APIErrorCode) {
	return ErrNone
}

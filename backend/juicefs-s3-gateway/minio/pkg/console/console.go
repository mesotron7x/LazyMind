...
var (
	...
	Printf = func(format string, data ...interface{}) {
		timeStr := fmt.Sprintf("time=\"%v\"", time.Now().UTC().Format(time.RFC3339))
		consolePrintf("Print", Theme["Print"], timeStr+" msg=\""+format+"\"\n", data...)
	}
	...
	Fatalf = func(format string, data ...interface{}) {
		timeStr := fmt.Sprintf("time=\"%v\"", time.Now().UTC().Format(time.RFC3339))
		consolePrintf("Fatal", Theme["Fatal"], timeStr+" msg=\""+format+"\"\n", data...)
		os.Exit(1)
	}
	...
	Errorf = func(format string, data ...interface{}) {
		timeStr := fmt.Sprintf("time=\"%v\"", time.Now().UTC().Format(time.RFC3339))
		consolePrintf("Error", Theme["Error"], timeStr+" msg=\""+format+"\"\n", data...)
	}
	...
	Infof = func(format string, data ...interface{}) {
		timeStr := fmt.Sprintf("time=\"%v\"", time.Now().UTC().Format(time.RFC3339))
		consolePrintf("Info", Theme["Info"], timeStr+" msg=\""+format+"\"\n", data...)
	}
	...
	Debug = func(data ...interface{}) {
		if os.Getenv(config.EnvLogDebugOn) != "on" {
			return
		}
	...
	Debugf = func(format string, data ...interface{}) {
		...
		timeStr := fmt.Sprintf("time=\"%v\"", time.Now().UTC().Format(time.RFC3339))
		consolePrintf("Debug", Theme["Debug"], timeStr+" msg=\""+format+"\"\n", data...)
	}

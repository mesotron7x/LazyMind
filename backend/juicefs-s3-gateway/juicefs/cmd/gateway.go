...
func redirectStdOutputToLogrus() {
	redirect := func(reader io.Reader, level logrus.Level) {
		scanner := bufio.NewScanner(reader)
		go func() {
			for scanner.Scan() {
				line := strings.TrimSpace(scanner.Text())
				if line != "" {
					logrus.WithField("source", "minio").Log(level, line)
				}
			}
		}()
	}
	...
	redirect(stdoutReader, logrus.InfoLevel)
	redirect(stderrReader, logrus.ErrorLevel)
}
...
func cmdGateway() *cli.Command {
	selfFlags := []cli.Flag{
		...
		&cli.StringFlag{
			Name:  "celery",
			Value: "",
			Usage: "use celery worker to perform file transforming",
		},
	}
...
func gateway(c *cli.Context) error {
	...
	app := &mcli.App{
		...
		Flags: []mcli.Flag{
			...
			mcli.StringFlag{
				Name:  "bucket-alias",
				Value: conf.Format.Name,
			},
			mcli.StringFlag{
				Name:  "celery",
				Value: jconfig.Celery,
			},
		},
		Writer:    os.Stdout,
		ErrWriter: os.Stderr,

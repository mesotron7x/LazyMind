...
func Main(args []string) error {
	...
	// 初始化 SigNoz OTLP gRPC 的 trace & log，上报到 OTel Collector。
	// 通过环境变量控制：
	// - SIGNOZ_OTLP_ENDPOINT: 必填，形如 "signoz-otel-collector:4317"；
	// - SIGNOZ_OTLP_INSECURE: 非空且不为 "false" 时使用明文（默认 true）；
	// - SIGNOZ_OTLP_TRACES_SAMPLED: 非空且不为 "false" 时开启采样（默认 true）；
	// - SIGNOZ_SERVICE_NAME: 覆盖默认服务名 "juicefs-s3-gateway"（可选）。
...
// initTelemetryFromEnv 根据环境变量初始化 SigNoz OTLP gRPC 的 trace 和 log。
func initTelemetryFromEnv() {
	endpoint := os.Getenv("SIGNOZ_OTLP_ENDPOINT")
	if endpoint == "" {
		// 未配置 endpoint，则认为不启用 Signoz / OTLP。
		endpoint = "signoz-otel-collector.signoz.svc.cluster.local:4317"
	}

	serviceName := os.Getenv("SIGNOZ_SERVICE_NAME")
	if serviceName == "" {
		// 默认按照部署名称设置服务名，便于在 SigNoz 中区分。
		serviceName = "juicefs-s3-gateway"
	}
	serviceVersion := version.Version()

	// 默认在 k8s 内网用明文传输，可通过 SIGNOZ_OTLP_INSECURE=false 切换为 TLS。
	insecure := true
	if v := strings.ToLower(os.Getenv("SIGNOZ_OTLP_INSECURE")); v == "false" || v == "0" {
		insecure = false
	}

	// 默认开启 trace 采样，可通过 SIGNOZ_OTLP_TRACES_SAMPLED=false 关闭。
	sampled := true
	if v := strings.ToLower(os.Getenv("SIGNOZ_OTLP_TRACES_SAMPLED")); v == "false" || v == "0" {
		sampled = false
	}

	_ = telemetry.InitTracerProvider(serviceName, serviceVersion, endpoint, insecure, sampled)
	_ = telemetry.InitLoggerProvider(serviceName, serviceVersion, endpoint, insecure)
}

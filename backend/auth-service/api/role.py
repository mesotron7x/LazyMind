...
def list_permission_groups(_: User = Depends(current_user)):
...
@permission_required('user.admin')
def list_roles(_: User = Depends(current_user)):
...
@permission_required('user.admin')
def create_role(body: RoleCreateBody, _: User = Depends(current_user)):
    """创建新角色，名称不可与已有角色重复"""

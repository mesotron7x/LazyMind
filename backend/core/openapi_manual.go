...
func manualSchemas() map[string]any {
	return map[string]any{
		...
		"SuspendJobRequest": objReq([]string{"name"}, prop("name", strSchema())),
		"UploadFileResponse": obj(prop("upload_file_id", strSchema()), prop("dataset_id", strSchema()), prop("filename", strSchema()), prop("stored_name", strSchema()), prop("stored_path", strSchema()), prop("relative_path", strSchema()), prop("document_pid", strSchema()), prop("document_tags", array(strSchema())), prop("file_size", int64Schema()), prop("content_type", strSchema()), prop("content_url", strSchema()), prop("download_url", strSchema()), prop("file_url", strSchema()), prop("status", strSchema())),
		...
		"InitUploadResponse": obj(prop("upload_id", strSchema()), prop("task_id", strSchema()), prop("document_id", strSchema()), prop("stored_name", strSchema()), prop("upload_mode", strSchema()), prop("part_size", int64Schema()), prop("total_parts", intSchema()), prop("upload_state", strSchema())),
		...
		"CompleteUploadResponse": obj(prop("task_id", strSchema()), prop("upload_id", strSchema()), prop("document_id", strSchema()), prop("stored_path", strSchema()), prop("parse_stored_path", strSchema()), prop("content_url", strSchema()), prop("download_url", strSchema()), prop("file_url", strSchema()), prop("file_size", int64Schema()), prop("convert_status", strSchema()), prop("convert_error", strSchema())),
...
func manualPaths() map[string]any {
	return map[string]any{
		...
		"/datasets/{dataset}/members/{member}": map[string]any{
			"get":    op("获取数据集成员", nil, nil, response(200, "成员详情", refSchema("DatasetMember"))),
			"delete": op("删除数据集成员", nil, nil, response(200, "删除成功", refSchema("EmptyObject"))),
			"patch":  op("更新数据集成员", nil, jsonBody(refSchema("UpdateDatasetMemberRequest"), true), response(200, "更新后的成员", refSchema("DatasetMember"))),
		},
		...
		"/datasets/{dataset}/tasks/{task}:initUpload": map[string]any{"post": op("初始化任务上传", nil, jsonBody(refSchema("InitUploadRequest"), true), response(200, "上传初始化结果", refSchema("InitUploadResponse")))},
		"/datasets/{dataset}/tasks/{task}/uploads/{upload_id}/parts/{part_number}": map[string]any{"put": binaryOp("上传分片", response(200, "分片上传结果", refSchema("UploadPartResponse")))},
		"/datasets/{dataset}/tasks/{task}/uploads/{upload_id}:complete": map[string]any{"post": op("完成上传", nil, jsonBody(refSchema("CompleteUploadRequest"), false), response(200, "完成上传结果", refSchema("CompleteUploadResponse")))},
		"/datasets/{dataset}/tasks/{task}/uploads/{upload_id}:abort": map[string]any{"post": op("中止上传", nil, jsonBody(refSchema("AbortUploadRequest"), false), response(200, "中止上传结果", refSchema("AbortUploadResponse")))},
